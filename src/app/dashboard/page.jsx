...
import { Button } from "@/components/ui/button_google";
import {
  Card,
  CardContent,
  CardHeader,
...
export default async function DashboardPage() {
  ...
  const activeOrders = validOrders.filter(
    (o) =>
      ...
      (o.status === "paid" || o.status === "escrow"),
  ).length;
  const pendingEscrow = validOrders
    .filter((o) => o.status === "escrow" && o.creator_id === user.id)
    .reduce((sum, o) => sum + o.amount, 0);
  const totalEarnings = validOrders
    .filter((o) => o.status === "released" && o.creator_id === user.id)
    .reduce((sum, o) => sum + o.amount, 0);

  // Client statistics
  const activePurchases = validOrders.filter(
    (o) =>
      o.client_id === user.id && (o.status === "paid" || o.status === "escrow"),
  ).length;
  const totalSpent = validOrders
    .filter(
      (o) =>
        o.client_id === user.id &&
        (o.status === "released" || o.status === "escrow"),
    )
    .reduce((sum, o) => sum + o.amount, 0);
  ...
  return (
    <div className="space-y-10">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        {!creatorProfile && (
          <Link href="/dashboard/profile">
            <Button
              variant="gradient"
              className="shadow-lg shadow-rose-500/20 px-8 py-6 rounded-2xl font-bold"
            >
      ...
      <Tabs
        defaultValue={userData?.role === "creator" ? "creator" : "client"}
        className="w-full"
      >
        <TabsList className="bg-slate-900/50 border border-white/10 p-1 rounded-full mb-8">
          <TabsTrigger
            value="creator"
            className="rounded-full px-8 data-[state=active]:bg-indigo-600 data-[state=active]:text-white"
          >
            As Creator
          </TabsTrigger>
          <TabsTrigger
      ...
      <Card className="glass border-white/5 bg-slate-900/40 backdrop-blur-xl rounded-3xl overflow-hidden mt-8">
        <CardHeader className="p-8 border-b border-white/5 bg-white/[0.01]">
          ...
          <CardDescription>
            Track your commission requests and escrow updates here.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-8">
          {validOrders.length === 0 ? (
            <div className="text-center py-16 px-4">
              ...
              <p className="text-slate-400 text-lg mb-8 max-w-md mx-auto">
                No recent activity. Start exploring or complete your profile to
            ...
            <div className="space-y-4">
              {validOrders.slice(0, 5).map((order) => {
                ...
                return (
                  <div
                    key={order.id}
                    className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-6 bg-slate-800/30 rounded-2xl border border-white/5"
                  >
                    <div className="mb-4 sm:mb-0">
                      <p className="text-white font-semibold">
                        Commission {isClient ? "with" : "for"} {otherPartyName}
                      </p>
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <span
                        className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${badgeColor}`}
                      >
                        {order.status}
                      </span>
                    </div>
