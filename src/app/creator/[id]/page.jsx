...
export default async function CreatorProfilePage({ params }) {
  ...
  const isImageLink = creator.portfolio_url?.match(
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between -mt-20 gap-6">
            <div className="flex items-end space-x-6">
              ...
              <div className="mb-2">
                ...
                <p className="text-indigo-400 font-medium font-mono text-sm leading-none mt-1">
                  @{creator.username}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mb-2">
              ...
              >
                <Button
                  variant="secondary"
                  className="px-8 py-3 text-base shadow-lg w-full sm:w-auto h-12 rounded-xl"
                >
              ...
              <Link href={`/order/${id}`}>
                <Button
                  variant="gradient"
                  className="px-8 py-3 text-base shadow-[0_0_20px_rgba(244,63,94,0.4)] w-full sm:w-auto h-12 rounded-xl"
                >
                  Hire via Escrow (Rp{" "}
                  {(creator.price_base || 0).toLocaleString("id-ID")})
                </Button>
              </Link>
            </div>
          </div>
